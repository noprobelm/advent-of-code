...
fn main() {
    let json_raw = file_to_string("input.txt").expect("Not a valid file");
    let json: Value = serde_json::from_str(&json_raw).expect("JSON was not well-formatted");

    let part_1 = part_1(&json);
    println!("The sum of all numbers found in the json array is {part_1}");

    let part_2 = part_2(&json);
    println!("The sum of all numbers found in the json array minus objects (and their children) who contain values of 'red' is {part_2}");
}
