...
fn main() {
    ...
    println!(
        "The highest cookie score among all possible {:?} combinations of ingredients is: {part_1}",
        combinations.len()
    );

    let part_2 = part_2(combinations.clone());
    println!("The highest cookie score among all possible {:?} combinations of ingredients which have a calorie score of exactly '500' is: {part_2}", combinations.len());
}
