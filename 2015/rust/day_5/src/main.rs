...
fn main() {
    ...
    println!("Part 1: {part_1} strings from the puzzle input are 'nice'");

    let part_2 = part_2(&lines);
    println!("Part 2: {part_2} strings from the puzzle input are 'nice'");
}
...
fn num_consecutive_letters(s: &String) -> usize {
    s.chars()
        .zip(s.chars().skip(1))
        .filter(|(x, y)| x == y)
        .count()
...
fn contains_blacklisted(s: &String) -> bool {
...
fn has_consecutive_with_divider(s: &String) -> bool {
...
fn has_repeating_no_overlap(s: &String) -> bool {
