...
fn main() {
    ...
    println!("There are {part_1} possible combinations of subsets of numbers from the puzzle input whose sum are equal to 150");

    let part_2 = part_2(&lines);
    println!("There are {part_2} possible combinations of containers when using the fewest number of contains possible from the puzzle input whose sum is 150");
}
