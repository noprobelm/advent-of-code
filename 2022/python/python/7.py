# This file is broken at the moment. The solution works fine, see
# https://github.com/noprobelm/filesystem-emulator for a working solution.
...
class System:
    ...
    def rm(self, fsobj: str) -> None:
        ...
        if path is self.root:
            self.stdout_buffer = (
                f":santa: Come on... You think Santa doesn't see you? ;) :santa:"
            )
            return
            ...
            if isinstance(child, Path):
                for _child, _child_obj in self.fstree.out_edges(child):
                    if isinstance(_child_obj, Path):
                        self.fstree.nodes[child][
                            "cumulative_size"
                        ] += self.fstree.nodes[_child_obj]["cumulative_size"]
                    elif isinstance(_child_obj, File):
                        self.fstree.nodes[child]["size"] += self.fstree.nodes[
                            _child_obj
                        ]["size"]
                        self.fstree.nodes[child][
                            "cumulative_size"
                        ] += self.fstree.nodes[_child_obj]["size"]
        ...
        if not as_tree:
            ...
            for node in self.fstree.nodes:
                if isinstance(node, Path):
                    stdout.append(
                        [
                            self.fstree.nodes[node]["cumulative_size"],
                            f"[blue]{node}[/blue]",
                        ]
                    )
                elif isinstance(node, File):
                    stdout.append(
                        [self.fstree.nodes[node]["size"], f"[red]{node}[/red]"]
                    )
            ...
            trees = {
                self.root: Tree(
                    f"{self.fstree.nodes[path]['cumulative_size']}\t[blue]{path}",
                    guide_style="blue",
                )
            }
            for node, edges in children:
                for edge_node in edges:
                    if isinstance(edge_node, Path):
                        ...
                        trees[node].add(
                            Tree(
                                f"{self.fstree.nodes[edge_node]['size']}\t[red]{edge_node.name}"
                            )
                        )
    ...
    def interactive(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                self.console.print(
                    "[red]Uh oh! You found a bug! I'm sure the elves will get right on it... :santa:"
                )
