...
def main():
    ...
    console.print(f"Part 1: {forest.num_visible}")

    highest_scoring = forest.highest_score
    console.print(f"Part 1: {highest_scoring[1]}")
...
class Forest(dict):
    ...
    def highest_score(self) -> tuple:
        highest_score = max([self[tree]["visibility_score"] for tree in self])
        highest_scoring_tree = list(
            filter(lambda tree: self[tree]["visibility_score"] == highest_score, self)
        )
